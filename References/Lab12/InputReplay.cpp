...
{
	// TODO: Reset initial player position/camera
	Player* player = mGame->GetPlayer();
	player->SetPosition(player->mInitialPos);
	player->SetRotation(0.0f);
	player->GetComponent<CameraComponent>()->ResetPitchAngle();
}
...
{
	return mGame->GetPlayer()->GetComponent<PlayerMove>()->GetVelocity();
}

const Vector3& InputReplay::GetPlayerAcceleration() const
{
	return mGame->GetPlayer()->GetComponent<PlayerMove>()->GetAcceleration();
}
...
{
	return mGame->GetPlayer()->GetComponent<CameraComponent>()->GetPitchAngle();
}
