...
{
	Actor* player = static_cast<Actor*>(mGame->GetPlayer());
	...
	for (int i = 0; i < mChannels.size(); i++)
	{
		if (mChannels[i].IsValid())
		{
			...
			{
				if (mHandleMap[mChannels[i]].mActor != nullptr)
					Mix_Volume(i, CalculateVolume(mHandleMap[mChannels[i]].mActor, player));
...
SoundHandle AudioSystem::PlaySound(const std::string& soundName, bool looping,
								   Actor* actor, bool stopOnActorRemove,
								   int fadeTimeMS)
{
	...
	newHandleInfo.mActor = actor;
	newHandleInfo.mStopOnActorRemove = stopOnActorRemove;

	mHandleMap[newSoundHandle] = newHandleInfo;
	mChannels[index] = newSoundHandle;
	if (actor != nullptr)
		mActorMap[actor].insert(newSoundHandle);
	...
	Mix_Volume(index, CalculateVolume(newHandleInfo.mActor, static_cast<Actor*>(mGame->GetPlayer())));
...
void AudioSystem::RemoveActor(Actor* actor)
{
	...
	{
		for (SoundHandle soundHandle : mActorMap[actor])
		{
			if (mHandleMap.find(soundHandle) != mHandleMap.end())
			{
				mHandleMap[soundHandle].mActor = nullptr;
				if (mHandleMap[soundHandle].mStopOnActorRemove &&
					Mix_Playing(mHandleMap[soundHandle].mChannel))
					Mix_HaltChannel(mHandleMap[soundHandle].mChannel);
			}
		}
...
// Caluclate Volume based on Distance
int AudioSystem::CalculateVolume(Actor* actor, Actor* listener) const
{
	...
	float soundRatio = 1 - Math::Clamp((distance - MIN_DISTANCE) / (MAX_DISTANCE - MIN_DISTANCE), 0.0f, 1.0f);

	return MIX_MAX_VOLUME * soundRatio;
}
