...
void LoadActor(const rapidjson::Value& actorValue, Game* game, Actor* parent)
{
	if (actorValue.IsObject())
	{
		...
		else if (type == "Prop")
		{
			...
			Prop* prop = new Prop(game, usesAlpha, usesCollision, meshStr);
			actor = prop;
		}
		...
		else if (type == "Door")
		{
			...
			Door* door = new Door(game, nameStr);
			actor = door;
		}
		else if (type == "EnergyLauncher")
		{
			...
			EnergyLauncher* launcher = new EnergyLauncher(game, cooldown, doorStr);
			actor = launcher;
		}
		else if (type == "EnergyCatcher")
		{
			...
			EnergyCatcher* catcher = new EnergyCatcher(game, doorStr);
			actor = catcher;
		}
